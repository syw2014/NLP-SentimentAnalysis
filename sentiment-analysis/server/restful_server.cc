...
static void handle_sum_call(struct mg_connection *nc, struct http_message *hm) {
  ...
  /* Get form variables */
  mg_get_http_var(&hm->body, "words", words, sizeof(words));

  /* Send headers */
  mg_printf(nc, "%s", "HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n");
    
  /* Compute the result and send it back as a JSON object */
  double score, label, s;
  score = label = s = 0.0;
 std::string content(words);
 //std::cout << "TEST: " << content << std::endl;
 label = pRating->PredictClassLabel(content, score);
  
 std::stringstream ss;
 std::string tmp("Evaluation Rejected");
 if(label == 1)
     tmp = "Positive";
 else if(label == -1)
     tmp = "Negative";
 if(score > 0.5)
     ...
     s = 1- score;
 //std::cout << "TEST label " << label << " score: " << s << std::endl;
 ss << tmp << ",Confidence  " << s;
...
static void ev_handler(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  switch (ev) {
    case MG_EV_HTTP_REQUEST:
      if (mg_vcmp(&hm->uri, "/api/v1/rating") == 0) {
